...
var Genetic = Genetic || (function(){
	...
	function Genetic() {
		...
		this.start = function() {
			...
			for (i=0;i<this.configuration.size;++i)  {
				var ent = this.seed();
		    if(Math.random() <= ent.speed / 55){
		      if(Math.random() <= ent.mouth / 11){
		        ent.fitness += 1;
						this.entities.push(Clone(ent));
		      }
		    }
			}
			...
			for (i=0;i<this.configuration.iterations;++i) {
				...
				for (var k = 0; k < newPop.length; k++ ){
					var ent = newPop[k];
					//// % chance indi will find food based on speed
			    if(Math.random() <= ent.speed / 55){ //50 * 1.1
			      //// % change indi will eat food base on mouthSize
			      if(Math.random() <= ent.mouth / 11){ //10 * 1.1
			        ent.fitness = 1; ////increment fitness 
							//this.entities.push(Clone(ent));
			      } else {
							newPop.splice(k, 1);
							k--;
